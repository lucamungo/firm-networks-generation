...
def generate_synthetic_network(N=1000, density=0.1, mu=0.0, sigma=1.0):
    ...
    # Generate random mask for sparsity
...
def create_config(properties, N, M=10, num_epochs=3000, num_cycles=1):
    ...
    config = {
        ...
        "loss_weights": {
            ...
            "hill": 1.0,
            "io": 5.0,
            "smooth": 1.0,  # Increased weight for smoothness
        },
        "training_phases": [
            {
                "name": "IO Matrix",
...
def main():
    ...
    if args.progressive:
        ...
        config, json_config = create_config(
            properties, N, num_epochs=args.epochs, num_cycles=args.num_cycles
        )
    else:
        print(f"Using standard training with {args.epochs} epochs")
        config, json_config = create_config(properties, N, num_epochs=args.epochs)
